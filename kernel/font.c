 ...
 * per scanline, MSB on the left. The table is indexed directly by ASCII
 * code (0-127). Control characters (0x00-0x1F) and DEL (0x7F) render as
 * blanks so the caller does not have to filter them out.
 */
static const uint8_t g_font_glyphs[128][FONT_GLYPH_HEIGHT] = {
